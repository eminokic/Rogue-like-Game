...
void place_lava_water(dungeon_t *d){
  int valid = 1, i, j, k, ran_x, ran_y, length, height;
  for (i = 0; i < 10; i++) {
      valid = 0;
    while(!valid) {
        valid = 1;
        ran_x = rand() % DUNGEON_X;
      ran_y = rand() % DUNGEON_Y;
      length = rand() % 6 + 2;
      height = rand() % 6 + 2;
        for (j = ran_y; j < ran_y + height; j++) {
          for (k = ran_x; k < ran_x + length; k++) {
            if (!in_room(d, j, k)) {
              valid = 0;
            }
          }
...
{
  /* Find the farthest two rooms, then connect                 *
...
int write_dungeon(dungeon_t *d, char *file)
{
  ...
  /* The Size of File, 4 bytes, 16-19 */
  ...
  /* The Rooms, num_rooms * 4 bytes, 16820-end */
  write_rooms(d, f);
  ...
  for (y = 0; y < DUNGEON_Y; y++) {
    for (x = 0; x < DUNGEON_X; x++) {
      ...
      if (d->hardness[y][x] == 0) {
         ...
         * after we've read the room array                             */
