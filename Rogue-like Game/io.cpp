...
uint32_t io_expunge_in(dungeon_t *d)
{
  ...
  for (i = 0; i < MAX_INVENTORY; i++) {
    /* We'll write 12 lines, 10 of inventory, 1 blank, and 1 prompt. *
     * We'll limit width to 60 characters, This gives offsets        *
...
void io_handle_input(dungeon_t *d)
{
  ...
  do {
...
#if 0
    case 'T':
      /* New!! Display distance for tunnelers.             */
      io_display_tunnel(d);
      fail_code = 1;
      break;
    case 'D':
      /* New!!  Display distance for non-tunnelers.         */
      io_display_distance(d);
      fail_code = 1;
      break;
    case 'H':
      /* New!!  Display hardness.                          */
      io_display_hardness(d);
      fail_code = 1;
      break;
    case 's':
      /* New!! After special screen, return to normal      */
      io_display(d);
      fail_code = 1;
      break;
#endif
    ...
    case 'q':
      ...
      /* Demonstrate use of the message queue.                       *
       * Using this command will waste a turn.                       *
       * waste a turn.                                               */
